...
```{r}
vfbconn=catmaid_login(server="https://fafb.catmaid.virtualflybrain.org")
...
plot3d(resample(dl1.fafb, stepsize = 1000))
```
...
```{r}
nclear3d()
plot3d(dl1.all, col=brain, soma=T)
plot3d(FAFB)
```
...
```{r}
nclear3d()
plot3d(dl1.all.r, col=side)
plot3d(FAFB)
```
```{r}
plot3d(dl1.all.r, WithConnectors=T)
```
